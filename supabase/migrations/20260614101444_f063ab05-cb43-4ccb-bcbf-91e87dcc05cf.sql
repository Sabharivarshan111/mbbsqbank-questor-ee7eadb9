...
AS $function$
  ...
  SELECT p.id, p.display_name, p.year::text,
   ...
   ORDER BY COALESCE(yc.year_xp, 0) DESC, p.xp DESC, p.streak DESC, p.display_name ASC
   LIMIT _limit;
$function$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
     WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='question_progress'
  ) THEN
    EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.question_progress';
  END IF;
END$$;
